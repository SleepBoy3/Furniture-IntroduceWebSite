        ...
        <script type="text/javascript" src="/JiaJu/manae/paging/jPages.js"></script>
         <script>
            ...
            function GongSiInfo_add(){
              ...
              content:'manae/AddGongSiInfo.jsp',
              end:function(layer,index){
              		},cancel:function(index,layero){//添加成功后自动刷新
              			layer.close(index);
              			location.href="/JiaJu/GongSiInfoServlet?method=All";
              		}
              	});
            }
           ...
           function GongSiInfo_upd(id) {
                 index=layer.open({
                          ...
                          maxmin:true,//最大化
                          area:['500px','500px'],//弹框的大小
                          content:'/JiaJu/GongSiInfoServlet?method=get&id='+id,//跳转
                          end:function(layer,index){	
            ...
            /*删除*/
            /* function GongSiInfo_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                    //发异步删除数据
                     $.post("/JiaJu/GongSiInfoServlet?method=delete",{
                    id:id
                    },function(res){
                    		if(res.success){
                    			alert(res.message);
           ...
           var name=document.getElementById("name").value;
           	if(name==""){
           		...
           		window.location.href="/JiaJu/GongSiInfoServlet?method=chaxun&name="+name;
           		}
...
.holder span{margin:0 5px;}
    ...
    <body>
        <div class="x-nav">
            ...
            <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"  href="/JiaJu/GongSiInfoServlet?method=All" title="刷新"><i class="layui-icon" style="line-height:30px">ဂ</i></a>
        </div>
        <div class="x-body">
                ...
                <div class="layui-form-pane" style="margin-top: 15px;">
                  <div class="layui-form-item"> 
                    ...
                    <div class="layui-input-inline">
                      <input type="text" name="name" id="name" placeholder="请输入标题名" autocomplete="off" class="layui-input">
                    </div>
            ...
            <%List<gongSiInfo> info=(List<gongSiInfo>)request.getAttribute("info"); //后台获取全部公司信息
            if(info!=null){
            %>
            <xblock>
          	<button class="layui-btn" onclick="GongSiInfo_add()"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" style="line-height:40px">共有数据：<%=info.size() %>条</span> 
            </xblock>
                ...
                <tbody id="FenYe">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <img src="<%=info.get(i).getUrl() %>" style="whidth:50px; height:80px;""/>
                            </u>
                        </td>
                        <td >
    					 <%=info.get(i).getTitle()%>       
                        ...
                        <td >
                           <%=info.get(i).getTelephone() %>
                        </td>
                        <td >
                           <%=info.get(i).getZsTelephone() %>
                        </td>
                         <td >
                           <%=info.get(i).getEmail() %>
                        </td>
                        ...
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="GongSiInfo_upd(<%=info.get(i).getId() %>)"
                            class="ml-5" style="text-decoration:none">
                                <i class="layui-icon">&#xe642;</i>
                            </a> 
                          <%--   <a title="删除" href="javascript:;" onclick="GongSiInfo_del(this,<%=info.get(i).getId() %>)" 
